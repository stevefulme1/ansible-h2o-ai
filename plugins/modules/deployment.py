...
"""Ansible module for managing H2O.ai deployment resources."""
...
---
module: deployment
short_description: Manage H2O.ai model deployments
description:
    - Create, update, and delete H2O.ai deployment resources.
version_added: "1.0.0"
author:
...
options:
    ...
    environment:
        description:
            - The environment for the H2O.ai resource.
        type: str
    state:
        description:
...
def find_resource(client, params):
    ...
    try:
        result = client.get("/api/v1/deployments")
        items = result if isinstance(result, list) else result.get("data", result.get("items", []))
        for item in items:
            if item.get("model_id") == params.get("model_id"):
                return item
    except Exception:
        pass
    return None
...
def create_resource(client, params):
    ...
    return client.post("/api/v1/deployments", data=payload)
...
def update_resource(client, existing, params):
    ...
    return client.put(f"/api/v1/deployments/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("deployment_id", ""))
    client.delete(f"/api/v1/deployments/{resource_id}")


def needs_update(params, existing):
    """Check if existing resource needs updating."""
    check_fields = ['name', 'environment']
