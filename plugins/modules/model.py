...
"""Ansible module for managing H2O.ai model resources."""
...
---
module: model
short_description: Manage H2O.ai models
description:
    - Create, update, and delete H2O.ai model resources.
version_added: "1.0.0"
author:
...
options:
    ...
    model_type:
        description:
            - The model type for the H2O.ai resource.
        type: str
    state:
        description:
...
RETURN = r"""
model:
    description: The model resource details.
...
def create_resource(client, params):
    ...
    return client.post("/api/v1/models", data=payload)


def update_resource(client, existing, params):
    """Update an existing resource."""
    resource_id = existing.get("id", existing.get("model_id", ""))
    payload = {k: v for k, v in params.items() if v is not None and k not in (
        "api_url", "api_key", "validate_certs", "timeout", "state",
    )}
    return client.put(f"/api/v1/models/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("model_id", ""))
    client.delete(f"/api/v1/models/{resource_id}")


def needs_update(params, existing):
    """Check if existing resource needs updating."""
    check_fields = ['model_type']
