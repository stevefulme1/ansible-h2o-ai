...
"""Ansible module for managing H2O.ai pipeline resources."""
...
---
module: pipeline
short_description: Manage H2O.ai ML pipelines
description:
    - Create, update, and delete H2O.ai pipeline resources.
version_added: "1.0.0"
author:
...
options:
    ...
    steps:
        description:
            - The steps for the H2O.ai resource.
        type: str
    description:
        description:
...
RETURN = r"""
pipeline:
    description: The pipeline resource details.
...
def create_resource(client, params):
    ...
    return client.post("/api/v1/pipelines", data=payload)


def update_resource(client, existing, params):
    """Update an existing resource."""
    resource_id = existing.get("id", existing.get("pipeline_id", ""))
    payload = {k: v for k, v in params.items() if v is not None and k not in (
        "api_url", "api_key", "validate_certs", "timeout", "state",
    )}
    return client.put(f"/api/v1/pipelines/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("pipeline_id", ""))
    client.delete(f"/api/v1/pipelines/{resource_id}")


def needs_update(params, existing):
    """Check if existing resource needs updating."""
    check_fields = ['steps', 'description']
...
def main():
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, pipeline=resource)
        return
    ...
    module.exit_json(changed=False, pipeline=existing)
