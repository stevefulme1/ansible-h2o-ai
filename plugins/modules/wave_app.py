...
"""Ansible module for managing H2O.ai wave_app resources."""
...
---
module: wave_app
short_description: Manage H2O Wave applications
description:
    - Create, update, and delete H2O.ai wave_app resources.
version_added: "1.0.0"
author:
...
options:
    ...
    title:
        description:
            - The title for the H2O.ai resource.
        type: str
    description:
        description:
    ...
    source_code_url:
        description:
            - The source code url for the H2O.ai resource.
        type: str
    state:
        description:
...
EXAMPLES = r"""
- name: Create a wave_app
  stevefulme1.h2o_ai.wave_app:
    api_url: "https://cloud.h2o.ai"
    api_key: "my-api-key"
    name: "example"
    state: present

- name: Delete a wave_app
  stevefulme1.h2o_ai.wave_app:
    api_url: "https://cloud.h2o.ai"
    api_key: "my-api-key"
    name: "example"
...
RETURN = r"""
wave_app:
    description: The wave_app resource details.
...
def create_resource(client, params):
    ...
    return client.post("/api/v1/wave_apps", data=payload)


def update_resource(client, existing, params):
    """Update an existing resource."""
    resource_id = existing.get("id", existing.get("wave_app_id", ""))
    payload = {k: v for k, v in params.items() if v is not None and k not in (
        "api_url", "api_key", "validate_certs", "timeout", "state",
    )}
    return client.put(f"/api/v1/wave_apps/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("wave_app_id", ""))
    client.delete(f"/api/v1/wave_apps/{resource_id}")


def needs_update(params, existing):
    """Check if existing resource needs updating."""
    check_fields = ['title', 'description', 'source_code_url']
...
def main():
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, wave_app=resource)
        return
    ...
    module.exit_json(changed=False, wave_app=existing)
